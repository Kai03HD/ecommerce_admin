...
export default function Products() {
  ...
  useEffect(() => {
    ...
    axios.get('/api/products').then(reponse => {
      setProducts(reponse.data);// Cập nhật state products với dữ liệu nhận được
    });
  }, []);// Chạy effect chỉ một lần sau khi component mount
      return (
      ...
      <Layout>
        <Link className="btn-primary" href="/products/new">Thêm sản phẩm mới</Link>
              ...
              {products?.map(product => (
                ...
                <tr key={product._id}>
                    <td>{product.title}</td>
                  <td><Link className="btn-default"  href={'/products/edit/'+product._id}>
